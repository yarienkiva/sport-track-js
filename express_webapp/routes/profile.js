...
router.post('/', function(req, res) {
	if (!req.session.authenticated) {
		...
		if (req.body.email == undefined || req.body.password == undefined || req.body.last_name == undefined ||  
			...
			res.status(500).render('error', {active: 'error', message: "Couldn't update user", error:{status: 500, stack: "A value wasn't enterred correctly"}});
		} else {
			...
			user_dao.update(new_user, function(err){
				if (err) res.status(500).render('error', {active: 'error', message: "Couldn't update user", error:{status: 500, stack: "A user with that email already exists"}});
				else {
